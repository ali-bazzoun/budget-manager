...
def save_budget_data(monthly_budget: MonthlyBudget):
    """
    Save the core budget data to the database using MonthlyBudget object.
    """
    user_id = monthly_budget.user_id
    month = monthly_budget.month
    income = monthly_budget.income
    category_percentages = monthly_budget.category_percentages
...
def get_budget_data(user_id: int, month: Optional[str] = None) -> Optional[MonthlyBudget]:
    """
...
def delete_budget(user_id: int, month: str) -> bool:
    """
...
def list_all_months_for_user(user_id: int) -> list:
    """
    List all months for which a user has budget data.
    """
    query = """--sql
